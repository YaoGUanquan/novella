...
| 统一生成能力     | operation/provider/model -> capability registry -> adapter/facade -> endpoint/task                                            | `src/core/services/ai/capability-registry.ts`、`src/core/services/ai/unified-generation-types.ts`                                                                                                                  |
...
- 上游 Novella 变更不得整体同步；插件 Registry、A2A 和离线手册分别按当前 Capability/Provider/Tauri 边界分层吸收。
- 上游插件代码不是当前运行时证据；A2A 必须补充 transport、认证、签名、schema、幂等和错误 ACK；离线手册必须审查能力声明证据。
- 图片资产读取必须限制到当前工作目录、合法项目 ID 和 `assets/images/` 前缀，并校验大小与图片魔数；不得用宽泛 asset protocol 暴露任意文件。
- 长时间生成任务必须保存创建时的 `GenerationExecutionContext`；轮询、取消和下载不得重新读取当前设置推导 provider/model。
